...
export default new class {
  ...
  /** 给主人发消息 */
  async sendMasterMsg (msg, id = Bot.uin) {
    if (Config.whole.notificationsAll) {
      // 发送全部管理
      for (let index of Config.masterQQ) {
        try { await this.relpyPrivate(id, index, msg) } catch { }
        await this.sleep(5000)
      }
    } else {
      // 发给第一个管理
      await this.relpyPrivate(id, Config.masterQQ[0], msg)
    }
  }
   ...
   * @param msg 消息
   */
  async relpyPrivate (id, userId, msg) {
    id = Number(id) || String(id)
    userId = Number(userId) || String(userId)

    let friend = Bot?.[id].fl.get(userId)
    if (friend) {
      logger.mark(`发送好友消息[${friend.nickname}](${userId})`)
      return await Bot[id].pickUser(userId).sendMsg(msg).catch((err) => {
        logger.mark(err)
      })
    }
  }
